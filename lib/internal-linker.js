...
// Check if text is inside an HTML header tag
function isInHeader(html, position) {
  // Check if we're inside an h1-h6 tag
  const beforeText = html.substring(0, position)
  const afterText = html.substring(position)

  // Look for opening header tag before position
  const headerOpenMatch = beforeText.match(/<h[1-6][^>]*>(?:(?!<\/h[1-6]>).)*$/s)
  if (!headerOpenMatch) return false

  // Check if there's a closing tag after position
  const headerCloseMatch = afterText.match(/^[^<]*<\/h[1-6]>/)
  return !!headerCloseMatch
}

// Check if text is already part of an HTML link
function isAlreadyLinked(html, position) {
  // Look backwards for <a
  const beforeText = html.substring(0, position)
  const linkOpenMatch = beforeText.match(/<a\s[^>]*>(?:(?!<\/a>).)*$/s)

  if (!linkOpenMatch) return false

  // Check if there's a closing </a> after position
  const afterText = html.substring(position)
  const linkCloseMatch = afterText.match(/^[^<]*<\/a>/)

  return !!linkCloseMatch
}
...
// Insert internal links into blog HTML
export function insertInternalLinks(blogHTML, maxLinks = 5) {
  ...
  // Extract key terms from blog (strip HTML first)
  const textContent = blogHTML.replace(/<[^>]+>/g, ' ')
  const blogTerms = extractTerms(textContent)
  ...
  for (const link of selectedLinks) {
    ...
    const linkText = `<a href="${link.url}">${originalText}</a>`
    updatedBlog =
